 ...
 * @since 7.0
 */
if (!is_admin()) add_action( 'wp_print_scripts', 'bizzthemes_theme_scripts' );
function bizzthemes_theme_scripts( ) { #hea

    // Compressed theme scripts
	// wp_enqueue_script( 'bizz-theme-dev', BIZZ_FRAME_SCRIPTS . '/theme.dev.js', array( 'jquery' ), '', true ); #footer
...
function localize_vars() {
	$lightbox = (isset($GLOBALS['opt']['bizzthemes_prettyphoto']['value'])) ? $GLOBALS['opt']['bizzthemes_prettyphoto']['value'] : false;
    $return = array(
        'siteurl' 	=> home_url(),
...
function bizzthemes_theme_head_styles( ) {
	...
	if ( !empty($GLOBALS['optd']['bizzthemes_layout_css']['value']) || !file_exists(TEMPLATEPATH . '/custom') ) {} else { // hide layout.css output
		// Layout stylesheet
		wp_register_style('layout_stylesheet',  get_template_directory_uri() .'/custom/layout.css?' . date('mdy-Gis', $date_modified_layout), '', '', 'screen, projection');
	...
	if ( !empty($GLOBALS['optd']['bizzthemes_custom_css']['value']) || !file_exists(TEMPLATEPATH . '/custom') ) {} else { // hide custom.css output
		// Custom stylesheet
		wp_register_style('custom_stylesheet', get_template_directory_uri() .'/custom/custom.css?' . date('mdy-Gis', $date_modified_custom), '', '', 'screen, projection');
		wp_enqueue_style('custom_stylesheet');
	}
	
	if ( isset($GLOBALS['opt']['bizzthemes_prettyphoto']['value']) ) {
		// Pretty Photo stylesheet
		wp_enqueue_style('bizz-fancybox', BIZZ_FRAME_SCRIPTS .'/fancybox/jquery.fancybox.css', false, '', 'screen');
	}
