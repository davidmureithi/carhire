...
jQuery(document).ready(function($) {
	...
	$( 'table.bizz_metabox tr:nth-last-child(1)' ).css( 'border', '0' );
	...
	$('.upload_button').live('click', function() {
		...
		tb_show('', 'media-upload.php?post_id=' + pID + '&type=image&bizz_force_send=true&bizz_send_label=' + buttonLabel + '&cbm_setting=cbm_value&TB_iframe=true');
		return false;
	});
    ...
    window.send_to_editor = function(html) {
		if (formfield) {
			...
			$('#' + formfield).siblings('.bizz_upload_status').slideDown().html(uploadStatus);
			tb_remove();
