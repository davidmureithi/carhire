...
class Bizz_Widget_Comments_Loop extends WP_Widget {
	...
	function Bizz_Widget_Comments_Loop() {
		$widget_ops = array( 'classname' => 'bizz-comments-loop', 'description' => __( 'Display Comments' ) );
		$control_ops = array( 'width' => 500, 'height' => 350, 'id_base' => 'bizz-comments-loop' );
		$this->WP_Widget( 'bizz-comments-loop', 'Comments', $widget_ops, $control_ops );
	}
...
<?php
	...
	function form( $instance ) {
		$defaults = array( 
			...
			'comment_moderation' => __( 'Your comment is awaiting moderation.' ),
			'password_text' => __( 'Password Protected' ),
			'pass_protected_text' => __( 'is password protected. Enter the password to view comments.' ),
			'sing_comment_text' => __( 'comment' ),
			'plu_comment_text' => __( 'comments' ),
			'sing_trackback_text' => __( 'trackback' ),
			'plu_trackback_text' => __( 'trackbacks' ),
			'sing_pingback_text' => __( 'pingback' ),
			'plu_pingback_text' => __( 'pingbacks' ),
			'sing_ping_text' => __( 'ping' ),
			'plu_ping_text' => __( 'pings' ),
			'no_text' => __( 'No' ),
			'to_text' => __( 'to' ),
			'req' => true, 
			'req_str' => __( '(required)' ), 
			...
			'logged_in_as' => __( 'Logged in as <a href="%s">%s</a>. <a href="%s" title="Log out of this account">Log out &raquo;</a>' ),
			'title_reply' => __( 'Leave a Reply' ), 
			'title_reply_to' => __( 'Leave a Reply to %s' ), 
			'cancel_reply_link' => __( 'Click here to cancel reply.' ), 
			'label_submit' => __( 'Submit Comment' ),
			'title_reply_tag' => 'h3',
			'comments_closed' => __( 'Comments are closed.' )
		);
...
?>
		<p>
			<label for="<?php echo $this->get_field_id( 'type' ); ?>"><?php _e( 'Comment type' ); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'comment_header' ); ?>"><?php _e( 'Comments headline tag' ); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title_reply_tag' ); ?>"><?php _e( 'Reply headline tag' ); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'comment_meta' ); ?>"><?php _e( 'Comment meta' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'max_depth' ); ?>"><?php _e( 'Max depth' ); ?></label>
		...
		<p>
			...
			<input class="checkbox" type="checkbox" <?php checked( $instance['req'], true ); ?> id="<?php echo $this->get_field_id( 'req' ); ?>" name="<?php echo $this->get_field_name( 'req' ); ?>" /> <?php _e( 'Require name and email'); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'enable_pagination' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['enable_pagination'], true ); ?> id="<?php echo $this->get_field_id( 'enable_pagination' ); ?>" name="<?php echo $this->get_field_name( 'enable_pagination' ); ?>" /> <?php _e( 'Enable pagination'); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'enable_reply' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['enable_reply'], true ); ?> id="<?php echo $this->get_field_id( 'enable_reply' ); ?>" name="<?php echo $this->get_field_name( 'enable_reply' ); ?>" /> <?php _e( 'Enable comment reply'); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'reverse_top_level' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['reverse_top_level'], true ); ?> id="<?php echo $this->get_field_id( 'reverse_top_level' ); ?>" name="<?php echo $this->get_field_name( 'reverse_top_level' ); ?>" /> <?php _e( 'Reverse the comment order'); ?></label>
		</p>
		<p>
			<br/><label><span class="translate"><?php _e( 'Translations'); ?></span></label>
		</p>
		<p>
			<label class="spb tog"><?php _e( 'comment list'); ?></label>
		</p>
		<p>
		...
		<p>
			<label class="spb tog"><?php _e( 'comment form'); ?></label>
		</p>
		<p>
...
function bizz_comments_number( $zero = false, $one = false, $more = false, $type = 'all', $comments ) {
	...
	elseif ( $number == 0 )
		...
		$output = ( false === $one ) ? __('One Comment') : $one;
...
function bizz_comment_time( $atts = array() ) {
	...
	$output = '<span class="comment-time"><abbr title="' . get_comment_date( __( 'g:i a' ) ) . '">' . get_comment_time() . '</abbr></span>';
...
function bizz_comment_link( $atts = array() ) {
	$defaults = array( 'before' => '', 'after' => '', 'label' => __( 'Permalink' ) );
	$args = shortcode_atts( $defaults, $atts );
	extract( $args, EXTR_SKIP );

	$output = '<span class="comment-permalink"><a href="' . esc_url(get_comment_link()) . '" title="' . sprintf( __( 'Permalink to %1$s %2$s' ), get_comment_type(), get_comment_ID() ) . '">' . $label . '</a></span>';
...
function bizz_comment_edit( $atts = array() ) {
	$defaults = array( 'before' => '', 'after' => '', 'label' => __( 'Edit' ) );
	$args = shortcode_atts( $defaults, $atts );
	extract( $args, EXTR_SKIP );
...
function bizz_comment_form( $form_args = array(), $post_id = null ) {
	...
	$form_args['comments_closed'] = ( isset($form_args['comments_closed']) ) ? $form_args['comments_closed'] :__( 'Comments are closed.' );
	$form_args['req'] = ( isset($form_args['req']) ) ? $form_args['req'] : true;
	$form_args['req_str'] = ( isset($form_args['req_str']) ) ? $form_args['req_str'] : __( '(required)' );
	$form_args['name'] = ( isset($form_args['name']) ) ? $form_args['name'] : __( 'Name' );
	$form_args['email'] = ( isset($form_args['email']) ) ? $form_args['email'] : __( 'Mail (will not be published)' );
	$form_args['url'] = ( isset($form_args['url']) ) ? $form_args['url'] : __( 'Website' );
	$form_args['must_log_in'] = ( isset($form_args['must_log_in']) ) ? $form_args['must_log_in'] : __( 'You must be <a href="%s">logged in</a> to post a comment.' );
	$form_args['logged_in_as'] = ( isset($form_args['logged_in_as']) ) ? $form_args['logged_in_as'] : __( 'Logged in as <a href="%s">%s</a>. <a href="%s" title="Log out of this account">Log out &raquo;</a>' );
	$form_args['title_reply_tag'] = ( isset($form_args['title_reply_tag']) ) ? $form_args['title_reply_tag'] : 'h3';
	$form_args['title_reply'] = ( isset($form_args['title_reply']) ) ? $form_args['title_reply'] : __( 'Leave a Reply' );
	$form_args['title_reply_to'] = ( isset($form_args['title_reply_to']) ) ? $form_args['title_reply_to'] : __( 'Leave a Reply to %s' );
	$form_args['cancel_reply_link'] = ( isset($form_args['cancel_reply_link']) ) ? $form_args['cancel_reply_link'] : __( 'Click here to cancel reply.' );
	$form_args['label_submit'] = ( isset($form_args['label_submit']) ) ? $form_args['label_submit'] : __( 'Submit Comment' );
...
?>
			...
			<?php else : ?>
				<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>">
					...
					<p>
						<button class="btn" name="submit" type="submit" id="submit" tabindex="<?php echo ( count( $args['fields'] ) + 2 ); ?>" value="<?php echo esc_attr( $args['label_submit'] ); ?>"><span><span><?php echo stripslashes(__('Add Comment')); ?></span></span></button>
