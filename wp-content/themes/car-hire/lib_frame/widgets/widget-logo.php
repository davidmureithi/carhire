...
class Bizz_Widget_Logo extends WP_Widget {
	...
	function Bizz_Widget_Logo() {
		$widget_ops = array( 'classname' => 'logo', 'description' => __( 'Control the output of your logotype.') );
		$this->WP_Widget( "widgets-reloaded-bizz-logo", __( 'Logo'), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		$custom_logo = array( 'def_logo' => __( 'Default logo (configure defult logo inside theme options panel)' ), 'def_title' => __( 'Default blog title &amp; tagline (configure Title &amp; Tagline inside Settings/General)' ), 'cus_logo' => __( 'Custom logo (upload custom logo for this widget only - setup below)' ) );
		...
		<p>
			<div class="wid_upload_wrap">
			    <label for="<?php echo $this->get_field_id( 'upload_logo' ); ?>"><?php _e( 'Upload custom logo:' ); ?></label>
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'custom_link' ); ?>"><?php _e( 'Choose a custom link:' ); ?></label>
