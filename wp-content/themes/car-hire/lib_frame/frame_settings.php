...
function bizzthemes_add_admin() {
	...
	// Theme Options Icon
	$adminmenu_icon 	= ( isset($GLOBALS['opt']['bizzthemes_branding_back_icon']['value']) ) ? $GLOBALS['opt']['bizzthemes_branding_back_icon']['value'] : '' ;
	$adminmenu_icon 	= ( isset($adminmenu_icon) && !empty($adminmenu_icon) ) ? $adminmenu_icon : '' ;
	$themeicon 			= ( !empty($adminmenu_icon) ) ? $adminmenu_icon : BIZZ_FRAME_IMAGES. '/bizzthemes-icon.png' ;
	
	// Initiate main theme options menu
	add_menu_page('Theme Options', $themename, 'edit_themes', $menu_function, $menu_options, $themeicon, 31);
...
function bizzthemes_add_submenus() {
	global $themeid, $themename, $frameversion;
	...
	if ( $adminmenu_layout != 'true' )
		add_submenu_page($menu_function, $themename, __('Builder'), 'manage_options', 'bizz-layout','bizzthemes_layout');
		
	add_submenu_page($menu_function, $themename, __('Framework'), 'manage_options', 'bizzthemes','bizzthemes_options');
	
	if ( $adminmenu_design != 'true' )
	    add_submenu_page($menu_function, $themename, __('Design'), 'manage_options', 'bizz-design','bizzthemes_options');
		
	if ( $adminmenu_editor != 'true' )
	    add_submenu_page($menu_function, $themename, __('Editor'), 'manage_options', 'bizz-editor', array('bizz_custom_editor', 'bizzthemes_editor'));
	
	if ( $adminmenu_license != 'true' && ( isset($_GET['page']) && $_GET['page'] == 'bizz-license' || $pending_license != '' ) )
	    add_submenu_page($menu_function, $themename, sprintf(__('License<span id="awaiting-mod" class="update-plugins"><span class="pending-count update-count">%s</span></span>'), $pending_license), 'manage_options', 'bizz-license','bizzthemes_license_page');
	
	if ( $adminmenu_tools != 'true' && isset($_GET['page']) && $_GET['page'] == 'bizz-tools' )
	    add_submenu_page($menu_function, $themename, __('Backup'), 'manage_options', 'bizz-tools', array('bizz_custom_tools', 'bizzthemes_tools'));
	
	if ( $adminmenu_version != 'true' && ( isset($_GET['page']) && $_GET['page'] == 'bizz-update' || $pending_update != '' ) )
	    add_submenu_page($menu_function, $themename, sprintf(__('Update<span id="awaiting-mod" class="update-plugins"><span class="pending-count update-count">%s</span></span>'), $pending_update), 'manage_options', 'bizz-update','bizzthemes_framework_update_page');
...
function bizzthemes_options() {
	...
	if ( $_GET['page'] == 'bizzthemes' ){
	    $options = $options;	
		$bizz_page = __('Framework Settings');
	} 
	elseif ( $_GET['page'] == 'bizz-design' ){
	    $options = $design;
		$bizz_page = __('Design Options');
	}
...
?>
					<div id="upload-wrap">
						<div class="upload_button" id="<?php echo $id; ?>"><?php _e('Choose File'); ?></div>
...
<?php
			...
			case "border":
				...
				if ( $sav != "") { $val = $sav; } elseif ( $sav2 != "") { $val = $sav2; } else { $val = $std; }
?>
				<input class="text_q q8 color {hash:true,caps:false,required:false}" name="<?php echo $value['id']; ?>[<?php echo $position; ?>-color]" id="<?php echo $value['id']; ?>_color" type="<?php echo $value['type']; ?>" value="<?php echo $val; ?>" />
				<input class="text_input" name="<?php echo $value['id']; ?>[css]" id="<?php echo $value['id']; ?>_css" type="hidden" value="<?php echo stripslashes(stripslashes($value['std']['css'])); ?>" />
<?php
				// border-width
				$position = $value['std']['border-position'];
...
function bizzthemes_options_header( $options_title = 'options', $toggle = false ) {
	...
	$return .= ( !empty($adminmenu_icon) ) ? '<div id="icon-themes" class="icon32"><br></div>' : '<div id="icon-bizzthemes" class="icon32"><br></div>';
	...
	$return .= '<a href="#" class="feedback"  onclick="window.open(\'http://bizzthemes.wufoo.com/forms/m7x4a3/\',  null, \'height=560, width=670, toolbar=0, location=0, status=1, scrollbars=1,resizable=1\'); return false" title="'.__('Give your Feedback').'">'.__('Report a Bug').'&nbsp;&nbsp;<img src="'. BIZZ_FRAME_IMAGES . '/bug.png" width="22" height="28" alt="Feedback" /></a>'."\n";
	$return .= '</h2>'."\n";
	
	if ( $toggle == true ) {
		$return .= '<div class="switch">'."\n";
		$return .= '<a id="master_switch" href="" title="'.__('Show/Hide All Options').'"><span class="pos">&nbsp;</span><span class="neg">&nbsp;</span> '.__('Show/Hide All Options').'</a>'."\n";
		$return .= '</div>'."\n";
	}
	$return .= '<div class="clear"><!----></div>'."\n";
	...
	if (!file_exists(TEMPLATEPATH . '/custom')) #Custom file alert
	    $return .= '<div id="message" class="updated fade themeinfo">'.__('<p>Rename your <code>custom-sample</code> folder to <code>custom</code>, otherwise theme will not work properly.</p>').'</div>'."\n";
	if (file_exists(TEMPLATEPATH . '/custom') && !is_writable(TEMPLATEPATH . '/custom'))
	    $return .= '<div id="message" class="updated fade themeinfo">'.__('<p>Warning: Your <code>custom</code> folder is not writeable. Please <a href="http://codex.wordpress.org/Changing_File_Permissions">CHMOD</a> all custom folder files to 777 restrictions, otherwise theme will not work properly.</p>').'</div>'."\n";
    if ( isset($_REQUEST['reset']) ) #Settings reset alert
	    $return .= '<div id="message" class="updated fade themeinfo">'.sprintf(__('<p>Settings reset!&nbsp; <a href="%1$s">Check out your website &rarr;</a></p>'), $bloghomeurl).'</div>'."\n";
		
	$return .= '<div class="clear"><!----></div>'."\n";
	$return .= '<div id="bizz-popup-save" class="save-popup">'.__('Changes Saved').'</div>'."\n";
...
function bizzthemes_options_footer() {
	...
	if ( $_GET['page'] == 'bizz-design' || $_GET['page'] == 'bizzthemes' ){
		...
		$return .= '<input name="save" type="submit" class="save_button" value="'.__('Save changes').'" />'."\n";
		$return .= '</div>'."\n";
	    $return .= '</form>'."\n";
	    $return .= '<form method="post">'."\n";
		$return .= '<div class="reset_save reset">'."\n";
		$return .= '<input name="reset" type="submit" class="save_button" value="'.__('Reset').'" onclick="return confirm(\''.__('All theme settings will be lost! Click OK to reset.').'\');" />'."\n";
...
function bizz_contextual_help($help, $screen_id, $screen) {
	...
	if ( is_admin() && ( 'admin.php' == $pagenow ) ) {
		...
		$return .= '<a href="http://codex.wordpress.org/" target="_blank">'.__('Documentation').'</a><br />';
		$return .= '<a href="http://wordpress.org/support/" target="_blank">'.__('Support Forums').'</a><br /><br />';
		$return .= '</div>';
		//add some new
		$return .= '<div class="metabox-prefs">';
		$return .= '<strong>BizzThemes</strong><br />';
		$return .= '<a class="optional" href="http://bizzthemes.com/documentation/" title="'.__('Read Theme Documentation & Installation Guide').'">'.__('Theme Documentation').'</a><br />';
		$return .= '<a class="optional" href="http://bizzthemes.com/forums/" title="'.__('Visit Support Forums').'">'.__('Theme Support Forums').'</a><br /><br />';
		$return .= '</div>';
	}
	else
		$return .= $help;
    ...
    echo '<div class="table-row">'."\n";
		...
		echo '<div class="top-container">'."\n";
			...
			if ( isset($values['desc']) ) {
			    ...
			    echo '<span class="trigger">'.__('more info [&#43;]').'</span>'."\n";
