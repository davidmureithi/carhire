...
function bizzthemes_framework_update_page(){
	...
	elseif ( $bizz_package == 'ZnJlZQ==' ) {
		...
		echo "\t\t<span style=\"display:none\">" . $method . "</span>\n";
			...
			echo "\t\t\t<div class=\"bizzalert_true\"><p>" . __("To use automatic framework updating system, please <a href='" . $bloghomeurl . "wp-admin/admin.php?page=bizz-license'>Upgrade to Standard or Agency Theme Package</a>.", 'bizzthemes') . "</p></div>\n";
		...
		echo "\t\t<form method=\"post\"  enctype=\"multipart/form-data\" id=\"bizzform\" action=\"\">\n";
			...
			echo "\t\t\t<ul class=\"file-download\">\n";
			echo "\t\t\t<li><a href=\"$remote_changelog\">View Theme Changelog</a></li>\n";
			echo "\t\t\t</ul>\n";
			}
			...
			if($this_theme_version < $remote_theme_version && $remote_theme_version != 'Currently Unavailable') {
				...
				echo "\t\t\t<div class=\"bizzalert_true\"><p>" . __('Theme update is available.', 'bizzthemes') . "</p></div>\n";
				...
				if ($remote_theme_version != 'Currently Unavailable')
					echo "\t\t\t<div class=\"bizzalert_false\"><p>" . __('You are using the latest theme version.', 'bizzthemes') . "</p></div>\n"; 
				...
				echo "\t\t\t</ul>\n";
				echo "\t\t\t<ul class=\"file-download\">\n";
				echo "\t\t\t<li><a href=\"$fw_remote_changelog\">View Framework Changelog</a></li>\n";
				echo "\t\t\t</ul>\n";
				...
				if($localversion < $remoteversion) {
					...
					echo "\t\t\t<div class=\"bizzalert_true\"><p>" . __('Framework update is available.', 'bizzthemes') . "</p></div>\n";
					...
					echo "\t\t\t<div class=\"bizzalert_false\"><p>" . __('You are using the latest framework version.', 'bizzthemes') . "</p></div>\n"; 
...
function bizzthemes_updates_check() {
	...
	if ( false === ( $remote_t_version = get_transient( 'remote_t_version_' . $themeid ) ) && !isset($_GET['activated']) ) {
		if ( bizz_is_online() ) {
			$remote_th_changelog = 'http://bizzthemes.com/demo/'.strtolower($themeid).'/wp-content/themes/'.strtolower($themeid).'/lib_theme/changelog.txt';
